...
#ifdef CONFIG_HUAWEI_KERNEL
	/* avoid the overflow which causes by large rbatt at 0 capacity */
	if ((u64)(abs(ibat_ua)) * (u64)(params->rbatt_mohm) > MAX_INT) {
...
#define DELTA_I_REPORT_100 -200000
	...
	if(new_calculated_soc == CAPACITY_99 && is_battery_charging(chip))
	{
		...
		/* avoid capacity falls back to 99 after reporting 100 during charging */
		if(chip->calculated_soc == CAPACITY_100)
		{
			chip->fake_soc_count = 0;
			new_calculated_soc = CAPACITY_100;
		}
	}
	else if(new_calculated_soc == CAPACITY_99 && !is_battery_charging(chip) && chip->calculated_soc == CAPACITY_100)
	{
		chip->fake_soc_count++;
		if(chip->fake_soc_count >= SAMPLE_COUNTER - 1)
		{
			chip->fake_soc_count = 0;
		}
		else
...
#ifdef CONFIG_HUAWEI_KERNEL
#define BAT_REMOVED_OFFMODE_CLEAR_REG	0x124F
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
		/* once the offmode bit is set, it won't be cleared unless we write the clear register */
		if (!rc && (batt_pres & BAT_REMOVED_OFFMODE_BIT)) {
			batt_pres = BAT_REMOVED_OFFMODE_BIT;
			qpnp_write_wrapper(chip, &batt_pres, BAT_REMOVED_OFFMODE_CLEAR_REG, 1);
			return true;
		}
#else
		if (!rc && (batt_pres & BAT_REMOVED_OFFMODE_BIT))
