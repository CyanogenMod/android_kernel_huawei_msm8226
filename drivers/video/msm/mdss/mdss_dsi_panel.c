...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (ctrl_pdata->panel_data.panel_info.bias_ic_enable)
		{
			if(ctrl_pdata->panel_data.panel_info.orise_ic)
			{
							...
							u32 bl_level)
{
	...
	case BL_DCS_CMD:
		...
		{
			LCD_LOG_INFO("%s:set brightness=%d\n",__func__,bl_level);
		}
		last_level = bl_level;
		break;
	default:
	...
	bool cmd_cfg_cont_splash)
{
	...
	if (!panel_name)
		...
		pinfo->orise_ic = (strstr(panel_name,"OTM1283A") ? true : false);
