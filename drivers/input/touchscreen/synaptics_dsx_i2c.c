		...
		struct device_attribute *attr, char *buf)
{
	...
	tp_log_debug("%s: rmi4_data->f01_ctrl_base_addr= 0x%x,glove_mode=%d", 
		__func__, rmi4_data->f01_ctrl_base_addr,glove_mode);
	/*Read doze wakeup threshold param for debug*/
	retval = synaptics_rmi4_i2c_read(rmi4_data,
	...
	return snprintf(buf, PAGE_SIZE, "flash_prog=%u\n"
		...
		data[0],data[1],device_data,no_sleep_setting,glove_mode,doze_wakeup_threshold,grip_algorithm_mode);
}
...
static int set_grip_algorithm_mode(struct synaptics_rmi4_data *rmi4_data, unsigned char mode)
{
	...
	if (ret < 0){
		...
		tp_log_info("%s: succeeded to set grip algorithm mode! grip_algorithm_mode = %d\n", 
	...
	if (!enable)
	{
		...
		*/
		...
		ret = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(force_cal));
		if (ret < 0) {
			tp_log_err("%s:failed read force_cal=%d!\n",
					__func__,force_cal);
			goto exit;
		} else{
			tp_log_info("%s:readforce_cal=%d!\n",
					__func__,force_cal);
		}
		force_cal |= 1 << 1;
		ret = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(force_cal));
		if (ret < 0) {
			tp_log_err("%s: Failed to set force_cal=%d!\n",
					__func__,force_cal);
			goto exit;
		} else{
			tp_log_info("%s:Set force_cal=%d!\n",
					__func__,force_cal);
		}
		/*the time is not be reduced,because it is in order to ensure ic work steadily*/
		msleep(10);
	}
