...
{
	struct ext4_super_block *es = EXT4_SB(sb)->s_es;
    /*when found system is broken, don't reboot as has no solution to handle now*/
    if(NULL != es->s_last_mounted){
        if(!strcmp(es->s_last_mounted, "/system")){
            printk("system partition is broken, please check\n");
            return;
        }
    }
