...
/* ratio is calculated the average value for 100cps modules, and changed the value to RATIO/256 */

/* average upward infinity for 100pcs modules is 266,
* average horizontial infinity for 100pcs modules is 214
* ratio is 214/266 = 204/256 (0.8)
*/
#define OV8858_OTP_INFINITY_UP_TO_HORIZON_RATIO 204
/* average upward macro for 100pcs modules is 576,
* average horizontial macro for 100pcs modules is 518
* ratio is 518/576 = 230/256 (0.9)
*/
#define OV8858_OTP_MACRO_UP_TO_HORIZON_RATIO 230
/* calculate the change ratio : up infinity to down infinity for 100pcs modules,
* Then select the min ratio as the OV8858_OTP_UP_TO_DOWN_RATIO in order to cover more modules.
* Then the min ratio is 0.5= 128 /256
*/
#define OV8858_OTP_INFINITY_UP_TO_DOWN_RATIO 128
#define OV8858_OTP_CHANGE_RATIO_DIV 8
...
static int RG_Ratio_Typical = 0x235;
static int BG_Ratio_Typical = 0x293;
...
* must be set to . After OTP memory access,set register 0x5002[3] back to
********************************************************************************************/
...
int32_t ov8858_read_group_data(struct msm_sensor_ctrl_t *s_ctrl, enum_ov8858_otp_type otp_type)
{
    ...
    {
        ov8858_otp_read_i2c(s_ctrl,start_address, &msb_data); //vcm infinity code msb
        ov8858_otp_read_i2c(s_ctrl,start_address + 1, &lsb_data); //vcm infinity code lsb
        g_ov8858_otp.VCM_infinity = ((msb_data & 0xFF) << 2) | ((lsb_data>>6) & 0x03);
...
int32_t ov8858_read_otp_data(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    g_ov8858_otp.rg_ratio,g_ov8858_otp.bg_ratio,g_ov8858_otp.gb_gr_ratio,g_ov8858_otp.VCM_infinity,g_ov8858_otp.VCM_end);
    //debug info test
...
/****************************************************************************
* FunctionName: ov8858_update_af_otp;
* Description : read AF otp data from sensor;
***************************************************************************/
int32_t ov8858_update_af_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
    //change the up_dac to horizontal_dac
    s_ctrl->sensordata->sensor_info->sensor_otp_afcalib.infinity_dac = (g_ov8858_otp.VCM_infinity*OV8858_OTP_INFINITY_UP_TO_HORIZON_RATIO)>>OV8858_OTP_CHANGE_RATIO_DIV;
    s_ctrl->sensordata->sensor_info->sensor_otp_afcalib.macro_dac= (g_ov8858_otp.VCM_end*OV8858_OTP_MACRO_UP_TO_HORIZON_RATIO)>>OV8858_OTP_CHANGE_RATIO_DIV;
    if(0 == g_ov8858_otp.VCM_start)
    {
      //if don't have staring dac in otp data,let staring dac is equal the down_infinity_dac
      s_ctrl->sensordata->sensor_info->sensor_otp_afcalib.starting_dac = (g_ov8858_otp.VCM_infinity*OV8858_OTP_INFINITY_UP_TO_DOWN_RATIO)>>OV8858_OTP_CHANGE_RATIO_DIV;
    }
	 pr_info("%s: horizon_infinity_dac = %d \n",__func__,s_ctrl->sensordata->sensor_info->sensor_otp_afcalib.infinity_dac);
	 pr_info("%s: horizon_macro_dac = %d \n",__func__,s_ctrl->sensordata->sensor_info->sensor_otp_afcalib.macro_dac);
	 pr_info("%s: starting_dac = %d \n",__func__,s_ctrl->sensordata->sensor_info->sensor_otp_afcalib.starting_dac);
    if((g_ov8858_otp.VCM_end <= g_ov8858_otp.VCM_infinity)
      ||(g_ov8858_otp.VCM_infinity <= g_ov8858_otp.VCM_start))
    {
      pr_err("%s:%d otp data error \n", __func__, __LINE__);
      return -1;
    }
    s_ctrl->sensordata->sensor_info->sensor_otp_af_calibration = 1;
    return 0;
}
/****************************************************************************
